...
/*   Updated: 2025/01/28 14:38:39 by esimpson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Character.hpp"

Character::Character() : _name(""), queue()
{
    for(int i=0; i < 4; ++i)
    {
        _inventory[i] = NULL;
...
Character::Character(std::string const &name) : _name(name), queue()
{
    for(int i=0; i < 4; ++i)
           _inventory[i] = NULL;
...
Character::~Character()
{
    ...
    while (!queue.isEmpty())
	{
		AMateria	*temp = (AMateria *)queue.dequeue();
		delete temp;
	}
}
